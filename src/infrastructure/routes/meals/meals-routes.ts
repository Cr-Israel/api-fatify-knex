...
export async function MealsRoutes(app: FastifyInstance) {
  ...
  app.patch('/meals/:mealId', { preHandler: [new CheckSessionIdExists().handler] }, new EditMeal().handler)
  app.delete('/meals/:mealId', { preHandler: [new CheckSessionIdExists().handler] }, new DeleteMeal().handler)
  app.get('/meals/:mealId', { preHandler: [new CheckSessionIdExists().handler] }, new GetMealById().handler)
